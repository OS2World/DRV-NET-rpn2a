...
public	semInt, semTx, semRx, semFlt, semMii, semStat, semInd, semReg
...
loc_2:
	...
	jnc	short loc_3	; Virtual/GDT address
...
RxData		proc	far	; TransferData
	...
	mov	dx,[bp+16]
	test	ax,-8
	jz	short loc_if		; null selector
	test	cx,-8
	jz	short loc_if
	test	dx,-8
	jz	short loc_if
	mov	ds,cx			; ds:si = TDFrameDesc
	mov	bx,[si].TDFrameDesc.TDDataCount
	dec	bx
	jl	short loc_ip		; zero count
	test	bh,0e0h
	jnz	short loc_ip		; too much counter
	shl	bx,3
loc_1:
	mov	di,[si+bx].TDFrameDesc.TDBufDesc1.TDDataLen
	test	di,di
	jz	short loc_ip		; zero length
	cmp	[si+bx].TDFrameDesc.TDBufDesc1.TDPtrType,0
	jz	short loc_2		; physical address

	cmp	[si+bx].TDFrameDesc.TDBufDesc1.TDPtrType,2
	ja	short loc_ip		; invalid pointer type
	dec	di
	add	di,word ptr [si+bx].TDFrameDesc.TDBufDesc1.TDDataPtr
	jc	short loc_ip		; address wrap around
	test	word ptr [si+bx].TDFrameDesc.TDBufDesc1.TDDataPtr[2],-8
	jz	short loc_ip		; null selector
loc_2:
	sub	bx,8			; sizeof(TDBufDesc)
	jnc	short loc_1
	...
	mov	ds,ax
	push	dx
	push	bx			; bytes copied
	push	di			; frame offset
	push	cx
	push	si			; TDFrameDesc
	call	_hwRxTransfer
	add	sp,5*2
...
RxRelease	proc	far	; RxRelease (Not Supported)
...
lock	btr	word ptr [MacStatus.sstMACstatus],msopen
	...
	push	offset semFlt
	call	_EnterCrit
	mov	[MCSTList.curnum],ax		; clear multicast
	mov	[MacStatus.sstRxFilter],ax	; clear rxfilter
	call	_LeaveCrit
	add	sp,2
	call	_hwClose
IF 0 
